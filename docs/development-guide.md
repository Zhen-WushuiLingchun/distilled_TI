...
3. 不依赖网络的 pairwise 比较器会对当前 choices 两两比较，给出确定性的倾向分布。
4. 融合层合并 LLM、embedding、pairwise 分布，返回 `source / confidence / option_scores / reason`。
5. 如果 `confidence >= GALGAME_FREE_TEXT_INFERENCE_MIN_CONFIDENCE`，`respond_galgame_scene()` 用推断出的 option 更新 scoring。
6. 如果分类失败或置信度低，继续使用用户显式选择的 option，不阻塞剧情。

离线校准集在 `backend/app/domain/galgame_calibration.py`，可用下面命令快速检查 pairwise-only 行为：

```powershell
python backend/scripts/galgame_text_calibration.py
```

### 2.0a `GalgameStoryTemplate`

剧情模板由 Admin 系统模板和用户个人模板两类组成：

- Admin 模板通过 `/api/admin/galgame/story-templates` 管理，`owner_user_id=null`。
- 用户模板通过公开接口 `/api/user/galgame/story-templates` 管理，必须带 `X-User-Id / X-User-Secret`。
- 用户模板保存 `owner_user_id`，只绑定随机匿名 ID，不包含真实姓名、手机号、邮箱或学校身份。
- `SessionService._select_galgame_story_template()` 会同时考虑系统模板和当前用户自己的模板，并按场景标签匹配。
- 模板内容只影响剧情包装，不直接改变评分、选题或聚类结果。
...
- 输出 `support_risk_flags`，作为非诊断的产品支持/人工复核提示
- 调用 AI 生成更有辨识度的命名与摘要

`support_risk_flags` 的边界必须明确：

- 它不是心理诊断，不输出“异常/正常”标签。
- 它只表达近期交互行为信号，例如快速作答、疲劳、一致性较低或极端选项比例较高。
- 它适合复用到 AI 助手聊天、在线咨询、校园服务等上下文项目里，作为“暂停、降温、转人工、追加澄清问题”的触发依据。
- 它不能作为惩罚、封禁、医学判断或自动干预依据。
- 如果未来用于高风险场景，需要增加明确同意、人工复核、危机资源提示和审计日志。
