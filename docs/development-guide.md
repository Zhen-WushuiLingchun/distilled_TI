...
6. 再由 `SessionService` 选择下一题
7. 达到最小题量后，`ScoringEngine.build_report()` + `ClusteringService` 输出结构化报告
...
### 7.4 如果引入 embedding / reranker，边界应该怎么划

如果后续按 ADR 引入向量层，推荐默认实现可以是：
...
因此 embedding / reranker 进入项目后，最合理的角色应该是：
...
1. 先做 `response_quality_score`
2. 再做 anchor / 镜像题一致性检测
3. 再把聚类样本池从“当前会话”扩到“持久化历史”
4. 最后再做更复杂的 AI 审阅器
