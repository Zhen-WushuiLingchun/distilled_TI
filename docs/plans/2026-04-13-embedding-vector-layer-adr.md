...
- `cluster_version`
- `cluster_index`
- `cluster_name_candidate`
- `narrative_label_candidate`
- `sample_size`
- `top_dimensions`
- `top_subdimensions`
- `top_modules`
...
在样本不够大时，簇本身可能不稳定，标签也会跟着漂移。

因此：

- 早期应采用“AI 候选 + 人工审核”的机制
...
4. 先服务于题目检索、AI 改写约束、会话近邻检索、自动簇命名
...
3. 最后才做 `cluster_vectors` 和自动簇命名

原因：

- 题目向量层最容易验证价值
- 会话向量层次之
- 自动簇命名最依赖样本规模和检索质量，适合后置
