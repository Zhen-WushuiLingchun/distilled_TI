...
未来建议不要把目标收窄为“固定 tag”，而是升级为“簇表达生成层”。
...
5. 由 LLM 生成：
   ...
   - `optional_short_name`
   - `evidence_points`
6. 后台管理端审核后生效，默认不自动覆盖当前簇标签

这样可以做到：

- 更符合项目“连续状态、非硬分型”的理念
- 输出不仅是固定标签，还包括可解释的关键词云和意象表达
- 更适合边界簇、混合簇和过渡簇
- 在保留自适应能力的同时，不完全失去可控性

进一步建议：

- `optional_short_name` 不是主输出，而是可选输出
- `cluster_summary + keyword_cloud + evidence_points` 才是主表达层
- 只有当簇样本量、簇稳定性和重复生成一致性都达到阈值时，才考虑半自动生效
...
在样本不够大时，簇本身可能不稳定，关键词云、解释和短名称也会一起漂移。

因此：

- 早期应采用“RAG + LLM 候选 + 人工审核”的机制
- 不应默认让 `optional_short_name` 自动覆盖现有簇标签
...
- 代表样本提取
- `keyword_cloud / cluster_summary / metaphor_candidates / optional_short_name`
- 后台审核流程
...
3. 优先采用 `Qdrant + 可替换 Embedding Provider` 方案
